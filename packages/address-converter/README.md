...
``` sh
npm install asa-address-converter
```
...
``` ts
import {ethToEthermint, ethermintToEth} from "asa-address-converter"
...
``` ts
import {ETH, ETHERMINT} from "asa-address-converter"
...
```ts
import {ethToEvmos, evmosToEth} from "asa-address-converter"
